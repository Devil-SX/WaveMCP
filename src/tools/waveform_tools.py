"""Unified waveform MCP tools using pywellen backend."""

from pathlib import Path

from mcp.server.fastmcp import FastMCP

from ..parsers import WellenParser, get_wellen_parser, set_wellen_parser


def register(mcp: FastMCP):
    """Register unified waveform tools with the MCP server."""

    @mcp.tool()
    async def load_waveform(path: str, max_file_size_mb: int = 100) -> str:
        """Load a waveform file for analysis (VCD and FST formats supported).

        Args:
            path: Path to the waveform file (relative or absolute)
    ...
    async def get_signals(
        ...
        """Get list of signals in the loaded waveform file with hierarchical filtering.
        ...
        lines = [f"Signals in waveform file (showing {len(signals)}/{total_count}{filter_info}):"]
        for sig in signals:
            lines.append(
    ...
    async def get_time_range() -> str:
        """Get the total time range of the loaded waveform."""
        try:
            parser = get_wellen_parser()
        except ValueError as e:
            return str(e)
    ...
    async def get_signal_values(
        ...
        """Get values for specified signals within a time range.
